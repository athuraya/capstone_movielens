...
# load the saved edx and validation sets from the saved files for consequent code runs (not the 1st time)
load("edx.RData")
...
# plot b_i to see the movie ratings' estimate variations
...
### Model 3: modeling for both movies and users effects
...
### Model 4: modeling for movies, users, and title effects

# Average rating of all titles
...
# Define a range of different values for lambda
lambdas <- seq(0, 15, 0.25)

# Use lambdas with RMSE regularize function
...
lambdas[which.min(regularize_RMSEs)]  # 4.75

##### Validate the outcome and results

# calculate the RMSE of actual ratings and predictions
RMSE_function <- function(actual_ratings, pred_ratings){
  sqrt(mean((actual_ratings - pred_ratings)^2))
}

# Validate RMSE of the best model 
M4_rmse <- RMSE_function(validation$rating, predicted_ratings_M4) 

M4_rmse  # same RMSE value as predicted_ratings_M4

# Get the minimum RMSE of the regularized model
